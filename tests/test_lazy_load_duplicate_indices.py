...
def test_lazy_load_duplicate_indices_no_overlap(tmp_path):
    ...
    for idx, header in enumerate(headers):
        assert header["profile_index"] == idx, (
            f"Header {idx} should have profile_index={idx}, "
            f"got {header['profile_index']}"
        )
    ...
    max_diff = np.max(np.abs(
        profiles_loaded[0].density - profiles_loaded[4].density
    ))
    assert max_diff > 0.3, (
        f"Profile 0 and 4 should differ significantly, "
        f"max diff={max_diff}"
    )
