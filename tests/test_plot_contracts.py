...
from linak.analysis.orientation import OrientationProfile
...
def test_position_profile_to_plot_data_contract_exposes_frame_atom_structure():
    ...
    assert contract.default_view_type_id == "line_1d"
...
def test_position_mapping_preset_distance_vs_time_uses_generic_roles():
    ...
    assert mapping.view_type_id == "line_1d"
...
def test_position_mapping_preset_x_z_trajectory_defaults_to_distance_color():
    ...
    assert mapping.view_type_id == "trajectory_2d"
    assert mapping.x == "x"
    assert mapping.y == "z"
...
def test_resolve_position_plot_mapping_validates_and_marks_profile_descriptor_mode():
    ...
    assert resolved.mapping.view_type_id == "trajectory_2d"
...
def test_density_heatmap_view_mapping_round_trips_mass_density():
    ...
    assert mapping.view_type_id == "heatmap_2d"
    assert mapping.fixed_values == {}
...
def test_potential_profiles_to_plot_data_contract_supports_line_and_table_views():
    profiles = [
        PotentialPlotSeries(
    ...
    assert [view.id for view in contract.view_types] == ["line_1d", "table_records"]


def test_potential_view_mapping_round_trips_table_view_and_summary_mode():
    table_mapping = potential_plot_options_to_view_mapping(table_view=True)
    table_options = potential_view_mapping_to_plot_options(table_mapping)
    assert table_options == {"view_type": "table_records"}
...
def test_resolve_potential_plot_mapping_validates_table_view_and_builds_rows():
    profiles = [
        PotentialPlotSeries(
            series_id="water_bulk_potential_ev",
            default_label="Water bulk",
    ...
    resolved = resolve_potential_plot_mapping(profiles=profiles, table_view=True)

    assert resolved.compatibility == "valid_preferred"
    assert resolved.mapping.view_type_id == "table_records"
    assert resolved.is_table_view is True
    assert resolved.view_type == "table_records"
    assert potential_table_rows(profiles) == [
        {
            "record_id": 1.0,
...
def test_orientation_heatmap_profile_to_plot_data_contract_exposes_distance_angle_grid():
    ...
    assert contract.view_types[0].id == "heatmap_2d"
...
def test_orientation_view_mapping_round_trips_line_and_heatmap_modes():
    ...
    assert line_mapping.fixed_values == {}
    line_options = orientation_view_mapping_to_plot_options(line_mapping)
    assert line_options == {"component": "density-weighted", "angle": "azimuthal"}
    ...
    assert heatmap_options == {"component": "heatmap", "angle": "polar"}
...
def test_resolve_orientation_plot_mapping_validates_line_and_heatmap_contracts():
    ...
    assert resolved_line.mapping.view_type_id == "line_1d"
    assert resolved_line.renderer_options == {"component": "average", "angle": "polar"}
    ...
    assert resolved_heatmap.mapping.view_type_id == "heatmap_2d"
    assert resolved_heatmap.renderer_options == {"component": "heatmap", "angle": "azimuthal"}
