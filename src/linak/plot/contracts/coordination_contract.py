"""Plot-data contract adapter for atom-resolved coordination profiles."""

from __future__ import annotations

from typing import TYPE_CHECKING

from ..data_contract import (
    PlotDataContract,
    PlotDimension,
    PlotQuantity,
    PlotViewType,
)

if TYPE_CHECKING:
    from ...analysis.coordination import CoordinationProfile


def _coordination_contract(
    *,
    source_id: str,
    label: str,
    axis_token: str,
    coordinate_mode: str,
    frame_length: int | None,
    atom_length: int | None,
) -> PlotDataContract:
    dimensions = (
        PlotDimension(
            id="frame",
            label="Frame",
            kind="time_index",
            length=None if frame_length is None else int(frame_length),
            unit="index",
        ),
        PlotDimension(
            id="atom",
            label="Atom",
            kind="entity_index",
            length=None if atom_length is None else int(atom_length),
            unit="index",
        ),
    )

    quantities = (
        PlotQuantity(
            id="frame_index",
            label="Frame index",
            kind="time_index",
            dimensions=("frame",),
            unit="index",
            source_name="frame_index",
        ),
        PlotQuantity(
            id="step",
            label="Step",
            kind="time_index",
            dimensions=("frame",),
            unit="step",
            source_name="step",
        ),
        PlotQuantity(
            id="time_fs",
            label="Time (fs)",
            kind="time",
            dimensions=("frame",),
            unit="fs",
            source_name="time_fs",
        ),
        PlotQuantity(
            id="time_ps",
            label="Time (ps)",
            kind="time",
            dimensions=("frame",),
            unit="ps",
            source_name="time_ps",
        ),
        PlotQuantity(
            id="atom_index",
            label="Atom index",
            kind="entity_index",
            dimensions=("atom",),
            unit="index",
            source_name="atom_indices",
        ),
        PlotQuantity(
            id="distance_to_surface",
            label=(
                "Distance to surface"
                if str(coordinate_mode).strip().lower() == "distance"
                else f"{axis_token.upper()} axis / distance field"
            ),
            kind="distance" if str(coordinate_mode).strip().lower() == "distance" else "coordinate",
            dimensions=("frame", "atom"),
            unit="Angstrom",
            source_name="distance_to_surface_A",
        ),
        PlotQuantity(
            id="coordination_number",
            label="Coordination number",
            kind="coordination",
            dimensions=("frame", "atom"),
            unit=None,
            source_name="coordination_number",
        ),
    )

    view_types = (
        PlotViewType(
            id="line_1d",
            label="Line 1D",
            kind="line_1d",
            supported_roles=("x", "y"),
        ),
        PlotViewType(
            id="scatter_2d",
            label="Scatter 2D",
            kind="scatter_2d",
            supported_roles=("x", "y", "color"),
        ),
        PlotViewType(
            id="trajectory_2d",
            label="Trajectory 2D",
            kind="trajectory_2d",
            supported_roles=("x", "y", "color"),
        ),
    )

    return PlotDataContract.from_items(
        source_id=str(source_id),
        label=str(label),
        dimensions=dimensions,
        quantities=quantities,
        view_types=view_types,
        default_view_type_id="line_1d",
    )


def default_coordination_plot_data_contract() -> PlotDataContract:
    """Return a generic coordination contract without profile-specific lengths."""

    return _coordination_contract(
        source_id="coordination:generic",
        label="Coordination data",
        axis_token="z",
        coordinate_mode="distance",
        frame_length=None,
        atom_length=None,
    )


def coordination_profile_to_plot_data_contract(profile: CoordinationProfile) -> PlotDataContract:
    """Convert one loaded ``CoordinationProfile`` into a ``PlotDataContract``."""

    axis_token = str(profile.axis).strip().lower() or "z"
    return _coordination_contract(
        source_id=(
            f"coordination:{profile.species_a}:{profile.species_b}:"
            f"{axis_token}:{profile.coordinate_mode}"
        ),
        label=f"Coordination profile: {profile.species_a}-{profile.species_b}",
        axis_token=axis_token,
        coordinate_mode=str(profile.coordinate_mode),
        frame_length=int(profile.n_frames),
        atom_length=int(profile.n_atoms),
    )
